...
async function requireEnv(name: string): Promise<string> {
  ...
  if (!v) {
    throw new Error(
      `[homefolio/auth] Missing ${name}. On Cloudflare Workers set it as a ` +
        `secret (\`wrangler secret put ${name} --name homefolio\`). Locally put ` +
...
export async function getAuth() {
  ...
  return betterAuth({
    ...
    // while `vp dev` serves on :3000 — we allow both so either entry point
    // works without editing env between runs. Production only ever hits
    // `BETTER_AUTH_URL`, so the extra localhost entries are harmless.
    ...
    databaseHooks: {
      user: {
        create: {
          before: async (user) => {
            // Derive a unique username and populate our app fields,
            // executed atomically in the same request as the Google OAuth
            // callback — so there's no window where a user exists in
